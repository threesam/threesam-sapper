...
export const day30 = (p5) => {
  ...
  p5.draw = () => {
    ...
    for (let plane = 0; plane < 5000; plane++) {
      const bounds = 2
      ...
      if (isCircle) {
        ...
        p5.rotate(rotation)
        p5.stroke(p5.map(plane, 0, 2000, 0, 255))
        makeHuman(x, y, color)
        p5.pop()
      }
  ...
  function makeHuman(x, y, color) {
    ...
    const whole = half * 2
    const limbLength = p5.random(30, 50)
    const minPossible = 0.025
    const torsoWidthOffset = 5//p5.random(5, quarter)
    const torsoLength = p5.random(30, 40)
    const neck = 0//p5.random(0, quarter / 8)
    p5.push()
    // head
    // if (p5.random() > minPossible) p5.rect(x - quarter, y - 100, whole, whole)
    if (p5.random() > minPossible) p5.ellipse(x, y - half - neck, quarter * 1.3, quarter * 1.3)
    // body
    ...
    p5.rotate(angle)
    if (p5.random() > minPossible) {
      p5.rect(x + half, y, quarter / 2, limbLength * p5.random(1, 1.3))
      // p5.line(x + half, y, x, y)
    }
    p5.rotate(-angle)
    if (p5.random() > minPossible) {
      p5.rect(x - quarter, y, quarter / 2, limbLength * p5.random(1, 1.3))
      // p5.line(x - quarter, y, x, y)

    }

    p5.translate(0, torsoLength)
    // legs
    if (p5.random() > minPossible) p5.rect(x + quarter, y, quarter / 3, limbLength * p5.random(1, 1.3))
    if (p5.random() > minPossible) p5.rect(x, y, quarter / 3, limbLength * p5.random(1, 1.3))
